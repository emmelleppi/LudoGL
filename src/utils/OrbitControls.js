...
export function OrbitControls(object, { element = sharedProps.canvas, enabled = true, target = new Vec3(), ease = 0.25, inertia = 0.85, enableRotate = true, rotateSpeed = 0.1, autoRotate = false, autoRotateSpeed = 1.0, enableZoom = true, zoomSpeed = 1, zoomStyle = 'dolly', enablePan = true, panSpeed = 0.1, minPolarAngle = 0, maxPolarAngle = Math.PI, minAzimuthAngle = -Infinity, maxAzimuthAngle = Infinity, minDistance = 0, maxDistance = Infinity } = {}) {
	...
	const onMouseDown = (e) => {
		...
		if (state !== STATE.NONE) {
			sharedProps.canvas.addEventListener('mousemove', onMouseMove, false);
			sharedProps.canvas.addEventListener('mouseup', onMouseUp, false);
		}
	};
	...
	const onMouseUp = () => {
		sharedProps.canvas.removeEventListener('mousemove', onMouseMove, false);
		sharedProps.canvas.removeEventListener('mouseup', onMouseUp, false);
		state = STATE.NONE;
	};
	...
	this.remove = function () {
		...
		sharedProps.canvas.removeEventListener('mousemove', onMouseMove);
		sharedProps.canvas.removeEventListener('mouseup', onMouseUp);
	};
