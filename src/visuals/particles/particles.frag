...
void main () {
  ...
  float emissiveAttenuationSoft =  1.0 / (2.0 + 6.0 * v_distFromNearestEmissive);
  float emissiveAttenuationMid = 1.0 / (2.0 + 6.0 * v_distFromNearestEmissive * v_distFromNearestEmissive);
  float emissiveAttenuationHard = 1.0 / (1.7 + 100.0 * v_distFromNearestEmissive * v_distFromNearestEmissive);

  vec3 albedo = SRGBToLinear(u_color);
  vec3 emissive = u_emissiveColor * (emissiveAttenuationSoft + emissiveAttenuationMid);
  emissive = mix(emissive, vec3(1.5), emissiveAttenuationHard);
